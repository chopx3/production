...
function drawQuestions(){ // отрисовка вопросов
	...
	for (var j=0; j<questQuantity; j++){ // от 0 до количества элементов в строке
		var idOfQuest = data[(activeQuestionsArray[i*4+j])].id; // для сокращения
		outputLine += 
		...
		'<input type="radio" name="question" id="quest-'+idOfQuest+'" autocomplete="off" value="'+idOfQuest+'">'+data[(activeQuestionsArray[i*4+j])].shortName + '</label>'; // отрисовка кнопки со значением и подписью
	}
