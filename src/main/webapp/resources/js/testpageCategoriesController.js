function openCategories(){
	document.getElementById("secondTable").innerHTML = ""; // обнуление инфы во второй таблице, дополнительной 
		drawInfo("cats"); // отрисовка дополнительных боковых кнопок
		$("#addWrapper").addClass("active").removeClass("higher"); // поставить их на обычные позиции
		$.get(getCategoriesURL).done(function (data) { // запрос к базе по всем агентам
			var info = data;
			var tbody =""; 
			var thead = '<div class="table-scroll col-lg-12"><table id="categoryTable" class="table table-striped table-hover"><thead><tr><th class="col-lg-1">id</th><th class="col-lg-5">description</th><th class="col-lg-3">shortname</th><th class="col-lg-2">isActive</th><th class="col-lg-1">edit</th></tr></thead><tbody>'; // шапка таблицы
