function openQuestions(){
	document.getElementById("secondTable").innerHTML = ""; // обнуление инфы во второй таблице, дополнительной 
		...
		$.get(getQuestionsURL).done(function (data) { // запрос к базе по всем агентам
			var info = data;
			var tbody =""; 
			var thead = '<div class="table-scroll col-lg-12"><table id="questionTable" class="table table-striped table-hover"><thead><tr><th class="col-lg-1">id</th><th class="col-lg-5">description</th><th class="col-lg-2">shortname</th><th class="col-lg-2">isActive</th><th class="col-lg-1">Position</th><th class="col-lg-1">edit</th></tr></thead><tbody>'; // шапка таблицы
			...
			for(var i=0;i<info.length;i++){ // для всех тэгов
				...
				dataArray = [id, description, shortname, position]; // массив, который будет передаваться
