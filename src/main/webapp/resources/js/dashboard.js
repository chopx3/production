...
function drawDashboardData(data){
	...
	for (var i = 0; i< react.length; i++){
		...
		reactBody+= '<tr><td>'+name+'</td><td class="centered">'+targetInHours+'</td><td class="centered" '+isReactionSlow+'>'+reactionHoursMinutes+'</td></tr>';
	}
	var satisfTableName = "Топ хороших оценок '4' и '5'";
	var satisfHead = '<div class="col-lg-6 pull-right"><table id="dashboardTableSatisfaction" class="table table-condensed table-hover" ><thead><tr><th colspan=3 class="col-lg-8">' + satisfTableName + '</th></tr></thead><tbody>';
	...
	for (var i=0; i<satisf.length; i++){
		...
		var progressBar = 
		  '<div class="progress">'+
			...
			'</div>'+
		  '</div>';
		satisfBody+='<tr ><td class="col-lg-5 noBottomBorderLine">'+satisf[i].name+'</td><td class="col-lg-1 noBottomBorderLine centered"">'+satisf[i].four+'/'+satisf[i].five+'</td><td class="col-lg-6 noBottomBorderLine">'+progressBar+'</td></tr>';
	}
